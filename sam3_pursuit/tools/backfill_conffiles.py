...
Usage:
    python tools/backfill_conffiles.py                    # default dataset (pursuit)
...
def main():
    ...
    parser.add_argument("--dataset", "-ds", default="pursuit", help="Dataset name (default: pursuit)")
    parser.add_argument("--masks-dir", help="Override masks directory (default: pursuit_masks)")
    parser.add_argument("--dry-run", action="store_true", help="Preview without writing files")
    args = parser.parse_args()

    base_dir = Path('.').resolve() #.parent
    db_path = base_dir / f"{args.dataset}.db"
    masks_dir = Path(args.masks_dir) if args.masks_dir else base_dir / f"pursuit_masks"
